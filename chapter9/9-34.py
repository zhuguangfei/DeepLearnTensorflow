...
class Seq2SeqModel(object):
    def __init__(
        ...
        def samplee_loss(labels, logits):
        ...
        softmax_loss_function = samplee_loss
        ...
        for i in range(buckets[-1][1] + 1):
            ...
            self.target_weights.append(
                tf.placeholder(dtype, shape=[None], name=f'weight{i}')
            )
        targets = [
            self.decoder_inputs[i + 1] for i in range(len(self.decoder_input) - 1)
        ]
        if forward_only:
            ...
            if not forward_only:
                ...
                for b in range(len(buckets)):
                    ...
                    clipped_gradients, norm = tf.clip_by_average_norm(
                        gradients, max_gradient_norm
                    )
                    self.gradients_norms.append(norm)
                    self.updates.append(
                        opt.apply_gradients(
    ...
    def get_batch(self, data, bucket_id):
        ...
        for length_idx in range(encoder_size):
            batch_encoder_inputs.append(
                np.array(
                    [
                        encoder_input[batch_idx][length_idx]
        ...
        batch_weights = np.ones(self.batch_size, dtype=tf.float32)
        for batch_idx in range(self.batch_size):
